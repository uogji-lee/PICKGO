...
- 방장은 참여 멤버에서 총무·길잡이·사진사 역할을 지정하거나 방장을 위임하고 멤버를 추방할 수 있습니다. 총무는 한 명이며, 방장도 회비 관리 권한을 가집니다. 길잡이와 사진사는 역할 표시용입니다.
- 추방 즉시 방 조회·수정·재입장을 차단합니다. 기존 납부·선결제·분담 내역은 보존되어 정산에 남습니다.
- 공동금고에서 1인당 곗돈을 설정하고 실제 입금액을 기록합니다. 부분 납부와 추가 납부를 지원합니다. 금액은 원 단위 정수입니다.
...
- 남은 추가 납부액 = 내 지출 분담액 − 낸 회비 − 개인 선결제 + 이미 반환받은 금액. 음수면 공동금고에서 돌려받습니다.
- 여행 종료 후 **최종 정산 시작**을 누르면 약정 회비 대신 실제 추가 납부액으로 미납 상태를 표시합니다. 이후 추가 지출도 자동으로 반영합니다.
- 추가 납부와 반환을 실제 송금한 뒤 확인 버튼으로 기록하면 남은 정산액이 줄어듭니다. 앱은 은행 송금을 실행하지 않습니다.
- 미납자에게 보내는 **회비 내라 👀** 알림은 방 내부 피드에만 표시되며, 방장·총무만 대상별 24시간에 한 번 보낼 수 있습니다. 공동금고의 새로고침 버튼으로 다른 멤버의 변경과 알림을 확인합니다.
- 잘못 입력한 납부·지출·반환 기록은 취소 후 다시 입력합니다. 이미 사용된 회비는 관련 지출 또는 반환 기록을 먼저 취소해야 합니다.
- 별도 추가 API 키는 필요 없습니다. 기존 SQLite DB에 새 컬럼과 테이블을 자동으로 추가합니다.
