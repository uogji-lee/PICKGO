async function loadKakaoPanel(container, room = null) {
  ...
  try {
    ...
    container.innerHTML = `<h2>💬 카카오 연결 · 친구</h2>
      ...
      ${config.enabled ? `<a class="kakao-login-link" href="/api/auth/kakao/start?mode=${config.linked ? 'friends' : 'link'}">${config.linked ? '카카오 친구 목록 동의' : '카카오 계정 연결'}</a>` : `<p class="desc">로그인 준비 중: ${config.missing.map(escapeHtml).join(', ')} 설정이 필요합니다.</p><details><summary>관리자 연결 안내</summary><p>카카오 로그인 활성화 후 다음 Redirect URI를 등록해주세요.</p><code>${escapeHtml(config.redirectUri)}</code><p>서버의 .env에 KAKAO_CLIENT_SECRET을 설정한 뒤 재시작해주세요.</p><a href="https://developers.kakao.com/console/app/1560656" target="_blank" rel="noopener noreferrer">카카오 개발자 설정 열기</a></details>`}
      ...
      <p data-social-status role="status"></p><ul class="finance-records" id="kakaoCandidates"></ul><div id="pickgoFriends"></div><div id="roomInvitations"></div>`;
    const status = text => { if (container.isConnected) el('[data-social-status]', container).textContent = text; };
    async function listFriends() {
      const { friends } = await api('/friends');
      if (!container.isConnected) return;
      el('#pickgoFriends', container).innerHTML = `<h3>내 PICKGO 친구</h3><ul class="finance-records">${friends.map(friend => `<li><span>${escapeHtml(friend.nickname)}</span>${room?.hostUserId === state.user.id ? `<button class="ghost small" data-invite-friend="${friend.id}">이 방에 초대</button>` : ''}</li>`).join('') || '<li>카카오 친구 목록에서 친구를 추가해주세요.</li>'}</ul>`;
