...
async function loadRoomFinance(room, members) {
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <details class="nested-collapsible" ${current ? 'open' : ''}><summary>🧾 지출 입력 · 추가 납부 요청</summary><div class="finance-section">
        ${canManage && current ? `<form id="expenseForm" class="finance-form"><label class="finance-wide">지출 내용<input name="title" maxlength="80" placeholder="숙소, 저녁, 렌터카…" required></label><label>금액<input name="amount" type="number" min="1" max="100000000" step="1" required></label><label>지출일<input name="date" type="date" value="${localDate()}" required></label><label class="finance-wide">결제한 돈<select name="payerUserId"><option value="pool">공동금고</option>${current.participantIds.map(id => `<option value="${id}">${name(id)} 개인 선결제</option>`).join('')}</select></label><fieldset class="finance-wide"><legend>같이 부담할 참석자</legend><div class="expense-participants">${current.participantIds.map(id => `<label><input type="checkbox" name="participantIds" value="${id}" checked>${name(id)}</label>`).join('')}</div></fieldset><button>지출 기록 · 참석자끼리 N빵</button></form>` : '<p class="desc">진행 중인 여행의 지출은 총무가 입력할 수 있습니다.</p>'}
      ...
      <details class="nested-collapsible" open><summary>📚 우리 방 여행 기록 (${trips.length}회)</summary><div class="finance-section">
        ${trips.map(trip => `<details class="journey-history"><summary>${escapeHtml(trip.title)} · ${trip.status === 'completed' ? '여행 완료' : '진행 중'} · ${won(trip.spent)}</summary><p>${escapeHtml(trip.plan.selected_date || '날짜 미정')} · ${tripLengthLabel(trip.plan.trip_nights)} · ${trip.participantIds.map(name).join(', ')}</p>
          <p class="desc">숙소 ${escapeHtml(trip.plan.accommodation_name || '미정')} · ${trip.plan.transport_mode === 'car' ? '차량' : '대중교통·도보'}</p>
          ${trip.settlement ? `<p class="desc">종료 시점 정산 · 이후 입금은 현재 잔액에서 확인하세요.</p><ul class="finance-records">${trip.settlement.people.map(person => `<li>${escapeHtml(person.nickname)} · 여행 부담 ${won(person.tripShare)} · ${person.balance < 0 ? `추가 납부 ${won(-person.balance)}` : `다음 여행 이월 ${won(person.balance)}`}</li>`).join('')}</ul>` : ''}
    ...
    form('#expenseForm', 'finance/expenses', values => ({ tripId: current.id, title: values.get('title'), amount: Number(values.get('amount')), date: values.get('date'), payerUserId: values.get('payerUserId') === 'pool' ? null : Number(values.get('payerUserId')), participantIds: values.getAll('participantIds').map(Number) }));
