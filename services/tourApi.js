...
function createTourApiClient(options = {}) {
  ...
  async function resolveRegionCodes(regionName) {
    ...
    const provinceBody = await request('ldongCode2', {
      numOfRows: 50,
      pageNo: 1,
      lDongListYn: 'N',
    }, CODE_CACHE_TTL_MS);
    ...
    codes.lDongSignguCd = String(district.code);
    return writeCache(cacheKey, codes, CODE_CACHE_TTL_MS);
  }

  async function getRecommendations(region, votes, limit = 6) {
    const codes = await resolveRegionCodes(region.name);
    const body = await request('areaBasedList2', {
      numOfRows: 100,
      pageNo: 1,
      arrange: 'Q',
      ...codes,
    });
    return rankTourItems(asArray(body.items?.item), votes, limit);
  }
